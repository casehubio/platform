...
public class SpringInMemoryGroupMembershipProvider implements GroupMembershipProvider {
    ...
    public void removeMember(String groupName, String tenancyId, String actorId) {
        ...
        if (group != null) {
            group.removeIf(m -> m.actorId().equals(actorId));
        }
    }
    ...
    public List<String> groupsOf(String actorId, String tenancyId) {
        ...
        for (var entry : members.entrySet()) {
            if (entry.getKey().startsWith(prefix)
                    && entry.getValue().stream().anyMatch(m -> m.actorId().equals(actorId))) {

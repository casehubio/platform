...
@ApplicationScoped
public class GoogleClient implements VendorClient {
    ...
    @Inject
    GoogleClient(ModelRegistry registry) {
        this.seedLookup = registry::query;
    }

    GoogleClient(Function<ModelQuery, List<ModelDescriptor>> seedLookup) {
        this.seedLookup = seedLookup;
    }

    @Override public String vendorKey() { return "google"; }
    @Override public String backendKey() { return "gemini"; }
    ...
    public ValidationResult listModels(Map<String, String> credentials) {
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                return ValidationResult.failure("Google API returned status " + response.statusCode());
            }
            return ValidationResult.success(parseModelsResponse(response.body()));
        } catch (Exception e) {
            return ValidationResult.failure("Failed to connect to Google API: " + e.getMessage());
        }
    }
    ...
    List<ModelDescriptor> parseModelsResponse(String json) {
        try {
            var seedModels = seedLookup.apply(
                ModelQuery.builder().vendor("google").build());
            ...
            for (JsonNode node : models) {
                ...
                String displayName = node.has("displayName") ? node.get("displayName").asText() : id;

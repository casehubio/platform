...
| `TemporalDriverService` | `@McpDomain("temporal-drivers")` — remote control API for `TemporalSimulationDriver`. Start/stop/pause/resume/setSpeed/status/list. Named profile references (from `TemporalProfileRegistry`) and inline profile definitions. COMPLETED/STOPPED drivers auto-replaced on start; RUNNING/PAUSED conflict errors. GraphQL + REST generated by APT. |
